...
install_wine_debian() {
    ...
    # On Ubuntu/Mint, WineHQ repo gives a more recent stable Wine
    ...
    if [[ "$wh_choice" =~ ^[Yy]$ ]]; then
        ...
        # Map Mint codenames back to Ubuntu base if needed
        case "$CODENAME" in
...
# ──────────────────────────────────────────────
# SET UP WINE PREFIX & DEPENDENCIES
# ──────────────────────────────────────────────
echo
...
WINEPREFIX=~/.multisim32 winetricks nocrashdialog -q corefonts mdac27 jet40
