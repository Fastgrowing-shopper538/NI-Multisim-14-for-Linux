...
install_wine_debian() {
    ...
    read -p "Add WineHQ official repo for latest stable Wine? (recommended) [Y/N]: " wh_choice
    if [[ "$wh_choice" =~ ^[Yy]$ ]]; then
        sudo apt-get install -y wget gpg
        sudo mkdir -pm755 /etc/apt/keyrings
        wget -O /tmp/winehq.key https://dl.winehq.org/wine-builds/winehq.key
        sudo gpg --dearmor -o /etc/apt/keyrings/winehq-archive.key /tmp/winehq.key

        # Detect codename
        CODENAME=$(. /etc/os-release; echo "${UBUNTU_CODENAME:-$VERSION_CODENAME}")

        # Map newer or unsupported codenames to an LTS release that has i386 packages
        case "$CODENAME" in
            resolute|kinetic|lunar) CODENAME="jammy" ;;   # force 22.04 LTS
            wilma|virginia|victoria|vera|una) CODENAME="jammy" ;;
            elsie|uma|ulyssa|ulyana) CODENAME="focal" ;;
        esac

        echo "Using WineHQ repo for Ubuntu $CODENAME (i386 + amd64 supported)"

        echo "deb [arch=amd64,i386 signed-by=/etc/apt/keyrings/winehq-archive.key] \
https://dl.winehq.org/wine-builds/ubuntu/ $CODENAME main" \
...
install_wine_suse() {
    ...
    SUSE_VER=$(. /etc/os-release; echo "$VERSION_ID")
...
# ──────────────────────────────────────────────
# CREATE PURE 32-BIT WINE PREFIX
# ──────────────────────────────────────────────
...
echo "Run it using your 32-bit Wine prefix:"
echo "  WINEPREFIX=~/.multisim32 wine ~/.wine/drive_c/Program\\ Files/National\\ Instruments/Multisim\\ 14.0/Multisim.exe"
